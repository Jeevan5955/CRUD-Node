...
export default class Hash {
  ...
  static generateToken(obj, expireTime) {
    return (expireTime) ? jwt.sign(obj, process.saarthi.app.key, { expiresIn: expireTime }) : jwt.sign(obj, process.saarthi.app.key);
  }
  ...
  static verifyToken(token) {
    return jwt.verify(token, process.saarthi.app.key);
  }
  ...
  static encrypt(data) {
    const cipher = crypto.createCipher(
      process.saarthi.hash.encrypt_algorithm,
      process.saarthi.app.key,
    );
